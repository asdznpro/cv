...
import { ChartLegend, ChartLegendOverlay } from '../legend'
import {
	buildChartCss,
...
function LineChartRoot<TData extends Record<string, unknown>>({
	...
	const buildOption = useCallback((): LineChartOption => {
	...
	useEffect(() => {
		...
		const push = (withEntrance: boolean) => {
			...
			chart.setOption(merged as LineChartOption, { notMerge: true })
			syncBrushOverlayNow()
		}
